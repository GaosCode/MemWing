...
def test_homebrew_formula_installs_release_artifact_without_placeholder_sha() -> None:
    ...
    assert (
        "https://github.com/GaosCode/MemWing/releases/download/v0.1.5/"
        "memwing-0.1.5.tar.gz"
    ) in formula
...
def test_release_artifact_bundles_python_dependencies_and_installs_to_prefix() -> None:
    ...
    assert 'VERSION="${MEMWING_VERSION:-0.1.5}"' in build_script
    ...
    assert 'VERSION="${MEMWING_VERSION:-0.1.5}"' in install_script
